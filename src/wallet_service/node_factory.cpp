...
namespace payment_service
{
    ...
    public:
        ...
        virtual void getRandomOutsByAmounts(std::vector<uint64_t> &&amounts, uint16_t outsCount,
                                            std::vector<mevacoin::RandomOuts> &result, const Callback &callback) override
        {
        }
    ...
    mevacoin::INode *NodeFactory::createNode(const std::string &daemonAddress, uint16_t daemonPort, uint16_t initTimeout, std::shared_ptr<logging::ILogger> logger)
    {
        std::unique_ptr<mevacoin::INode> node(new mevacoin::NodeRpcProxy(daemonAddress, daemonPort, initTimeout, logger));
