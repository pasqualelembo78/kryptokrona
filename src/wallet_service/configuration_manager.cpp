...
namespace payment_service
{
    ...
    bool ConfigurationManager::init(int argc, char **argv)
    {
        ...
        {
            std::cout << mevacoin::getProjectCLIHeader() << asString(serviceConfig) << std::endl;
            exit(0);
        }
        else if (!serviceConfig.outputFile.empty())
        {
            try
            {
                ...
                std::cout << mevacoin::getProjectCLIHeader() << "Configuration saved to: " << serviceConfig.outputFile << std::endl;
                exit(0);
            }
            catch (std::exception &e)
            {
                std::cout << mevacoin::getProjectCLIHeader() << "Could not save configuration to: " << serviceConfig.outputFile
                          << std::endl
                          << e.what() << std::endl;
                exit(1);
            }
