...
namespace
{
    ...
    std::string printTransactionShortInfo(const mevacoin::CachedTransaction &transaction)
    {
        std::stringstream ss;
    ...
    std::string printTransactionFullInfo(const mevacoin::CachedTransaction &transaction)
    {
        std::stringstream ss;
        ss << printTransactionShortInfo(transaction);
        ss << "JSON: \n"
           << mevacoin::storeToJson(transaction.getTransaction()) << std::endl;
...
DaemonCommandsHandler::DaemonCommandsHandler(
    mevacoin::Core &core,
    mevacoin::NodeServer &srv,
    std::shared_ptr<logging::LoggerManager> log,
    mevacoin::RpcServer *prpc_server) : m_core(core),
                                          m_srv(srv),
                                          logger(log, "daemon"),
                                          m_logManager(log),
...
{
    std::stringstream ss;
    ss << mevacoin::MEVACOIN_NAME << " v" << PROJECT_VERSION << ENDL;
    ss << "Commands: " << ENDL;
...
bool DaemonCommandsHandler::print_bc(const std::vector<std::string> &args)
{
    ...
    mevacoin::COMMAND_RPC_GET_BLOCK_HEADERS_RANGE::request req;
    mevacoin::COMMAND_RPC_GET_BLOCK_HEADERS_RANGE::response res;
    mevacoin::json_rpc::JsonRpcError error_resp;
    ...
    const mevacoin::Currency &currency = m_core.getCurrency();

    bool first = true;
    for (mevacoin::block_header_response &header : res.headers)
    {
        if (!first)
        {
            std::cout << ENDL;
...
bool DaemonCommandsHandler::print_tx(const std::vector<std::string> &args)
{
    ...
    std::vector<mevacoin::BinaryArray> txs;
    std::vector<crypto::Hash> missed_ids;
    m_core.getTransactions(tx_ids, txs, missed_ids);

    if (1 == txs.size())
    {
        mevacoin::CachedTransaction tx(txs.front());
...
bool DaemonCommandsHandler::print_pool(const std::vector<std::string> &args)
{
    ...
    {
        mevacoin::CachedTransaction ctx(tx);
...
{
    mevacoin::COMMAND_RPC_GET_INFO::request ireq;
    mevacoin::COMMAND_RPC_GET_INFO::response iresp;
