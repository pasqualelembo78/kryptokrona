...
namespace mevacoin
{
    ...
    RpcServer::RpcServer(syst::Dispatcher &dispatcher, std::shared_ptr<logging::ILogger> log, Core &c, NodeServer &p2p, IMevaCoinProtocolHandler &protocol) : HttpServer(dispatcher, log), logger(log, "RpcServer"), m_core(c), m_p2p(p2p), m_protocol(protocol)
    {
    }
    ...
    bool RpcServer::on_get_info(const COMMAND_RPC_GET_INFO::request &req, COMMAND_RPC_GET_INFO::response &res)
    {
        ...
        res.upgrade_heights = mevacoin::parameters::FORK_HEIGHTS_SIZE == 0 ? std::vector<uint64_t>() : std::vector<uint64_t>(mevacoin::parameters::FORK_HEIGHTS, mevacoin::parameters::FORK_HEIGHTS + mevacoin::parameters::FORK_HEIGHTS_SIZE);
        res.supported_height = mevacoin::parameters::FORK_HEIGHTS_SIZE == 0 ? 0 : mevacoin::parameters::FORK_HEIGHTS[mevacoin::parameters::CURRENT_FORK_INDEX];
        res.hashrate = (uint32_t)round(res.difficulty / mevacoin::parameters::DIFFICULTY_TARGET);
    ...
    bool RpcServer::f_on_transaction_json(const F_COMMAND_RPC_GET_TRANSACTION_DETAILS::request &req, F_COMMAND_RPC_GET_TRANSACTION_DETAILS::response &res)
    {
        ...
        if (mevacoin::getPaymentIdFromTxExtra(res.tx.extra, paymentId))
        {
    ...
    bool RpcServer::on_getblocktemplate(const COMMAND_RPC_GETBLOCKTEMPLATE::request &req, COMMAND_RPC_GETBLOCKTEMPLATE::response &res)
    {
        ...
        PublicKey tx_pub_key = mevacoin::getTransactionPublicKeyFromExtra(blockTemplate.baseTransaction.extra);
    ...
    bool RpcServer::on_get_block_headers_range(const COMMAND_RPC_GET_BLOCK_HEADERS_RANGE::request &req, COMMAND_RPC_GET_BLOCK_HEADERS_RANGE::response &res, json_rpc::JsonRpcError &error_resp)
    {
        ...
        for (uint32_t h = static_cast<uint32_t>(req.start_height); h <= static_cast<uint32_t>(req.end_height); ++h)
        {
            ...
            mevacoin::BlockTemplate blk = m_core.getBlockByHash(block_hash);
