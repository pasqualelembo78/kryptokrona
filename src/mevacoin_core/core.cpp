...
#include <mevacoin_core/blockchain_cache.h>
#include <mevacoin_core/blockchain_storage.h>
#include <mevacoin_core/blockchain_utils.h>
#include <mevacoin_core/core.h>
#include <mevacoin_core/core_errors.h>
...
#include <mevacoin_core/memory_blockchain_storage.h>
#include <mevacoin_core/mixins.h>
#include <mevacoin_core/transaction_api.h>
#include <mevacoin_core/transaction_extra.h>
#include <mevacoin_core/transaction_pool.h>
#include <mevacoin_core/transaction_pool_cleaner.h>
#include <mevacoin_core/upgrade_manager.h>

#include <mevacoin_protocol/mevacoin_protocol_handler_common.h>
...
namespace mevacoin
{
    ...
    namespace
    {
        ...
        TransactionValidatorState extractSpentOutputs(const CachedTransaction &transaction)
        {
            ...
            const auto &mevacoinTransaction = transaction.getTransaction();

            for (const auto &input : mevacoinTransaction.inputs)
            {
                if (input.type() == typeid(KeyInput))
                {
        ...
        const mevacoin::Transaction &t)
    {
        ...
        for (const auto &output : t.outputs)
        {
            ...
            keyOutput.key = boost::get<mevacoin::KeyOutput>(output.target).key;
        ...
        const std::vector<uint8_t> &rawTX)
    {
        ...
        {
            transaction.keyInputs.push_back(boost::get<mevacoin::KeyInput>(input));
        }
    ...
    */
    ...
    std::error_code Core::addBlock(const CachedBlock &cachedBlock, RawBlock &&rawBlock)
    {
        ...
        if (blockIndex >= mevacoin::parameters::BLOCK_BLOB_SHUFFLE_CHECK_HEIGHT)
        {
        ...
        if (mixinChangeWindow > mevacoin::parameters::MEVACOIN_MINED_MONEY_UNLOCK_WINDOW)
        {
            mixinChangeWindow = mixinChangeWindow - mevacoin::parameters::MEVACOIN_MINED_MONEY_UNLOCK_WINDOW;
        }
    ...
    bool Core::isTransactionValidForPool(const CachedTransaction &cachedTransaction, TransactionValidatorState &validatorState)
    {
        ...
        if (cachedTransaction.getTransaction().extra.size() >= mevacoin::parameters::MAX_EXTRA_SIZE_POOL)
        {
    ...
    std::tuple<bool, mevacoin::BinaryArray> Core::getPoolTransaction(const crypto::Hash &transactionHash) const
    {
        if (transactionPool->checkIfTransactionPresent(transactionHash))
        {
                                ...
                                uint64_t &difficulty, uint32_t &height) const
    {
        ...
        {
            blockchain_timestamp_check_window = mevacoin::parameters::BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW_V3;
        }
        else
        {
            blockchain_timestamp_check_window = mevacoin::parameters::BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW;
        }
        ...
        for (const auto &input : transaction.inputs)
        {
            if (input.type() == typeid(KeyInput))
            {
                ...
                if (!checkpoints.isInCheckpointZone(blockIndex + 1))
                {
                    ...
                    if (blockIndex >= mevacoin::parameters::TRANSACTION_SIGNATURE_COUNT_VALIDATION_HEIGHT && outputKeys.size() != cachedTransaction.getTransaction().signatures[inputIndex].size())
                    {
    ...
    std::vector<crypto::Hash> mevacoin::Core::getBlockHashes(uint32_t startBlockIndex, uint32_t maxCount) const
    {
    ...
    std::error_code Core::validateBlock(const CachedBlock &cachedBlock, IBlockchainCache *cache, uint64_t &minerReward)
    {
        ...
        if (cachedBlock.getBlockIndex() >= mevacoin::parameters::TRANSACTION_SIGNATURE_COUNT_VALIDATION_HEIGHT && !block.baseTransaction.signatures.empty())
        {
    ...
    uint64_t mevacoin::Core::getAdjustedTime() const
    {
        return time(NULL);
    }
        ...
        const uint64_t blockHeight) const
    {
        ...
        if (transaction.extra.size() >= mevacoin::parameters::MAX_EXTRA_SIZE_BLOCK)
        {
            logger(logging::TRACE) << "Not adding transaction "
        ...
        uint64_t &fee) const
    {
        ...
        for (auto it = poolTransactions.rbegin(); it != poolTransactions.rend() && it->getTransactionFee() == 0; ++it)
        {
            ...
            if (!validateBlockTemplateTransaction(transaction, height))
            {
                ...
                if (transactionAge >= mevacoin::parameters::MEVACOIN_MEMPOOL_TX_LIVETIME)
                {
