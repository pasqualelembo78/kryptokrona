...
namespace mevacoin
{
    ...
    NodeServer::NodeServer(syst::Dispatcher &dispatcher, mevacoin::MevaCoinProtocolHandler &payload_handler, std::shared_ptr<logging::ILogger> log) : m_dispatcher(dispatcher),
...
#undef INVOKE_HANDLER
    ...
    //-----------------------------------------------------------------------------------
    void NodeServer::for_each_connection(std::function<void(MevaCoinConnectionContext &, uint64_t)> f)
    {
    ...
    void NodeServer::externalRelayNotifyToList(int command, const BinaryArray &data_buff, const std::list<boost::uuids::uuid> relayList)
    {
        ...
        if (std::find(relayList.begin(), relayList.end(), conn.m_connection_id) != relayList.end()) {
          if (conn.peerId && (conn.m_state == MevaCoinConnectionContext::state_normal ||
    ...
    bool NodeServer::handle_remote_peerlist(const std::list<PeerlistEntry> &peerlist, time_t local_time, const MevaCoinConnectionContext &context)
    {
        int64_t delta = 0;
