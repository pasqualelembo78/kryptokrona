...
namespace utilities
{
        ...
        const uint64_t supported_height)
    {
        ...
        const float days = (next_fork - height) / mevacoin::parameters::EXPECTED_NUMBER_OF_BLOCKS_PER_DAY;
        ...
        const std::vector<uint64_t> upgrade_heights)
    {
        ...
        const float days = (static_cast<float>(next_fork - height) / mevacoin::parameters::EXPECTED_NUMBER_OF_BLOCKS_PER_DAY);
        ...
        const std::vector<uint64_t> upgrade_heights)
    {
        for (auto upgrade : upgrade_heights)
        {
            ...
            {
                return "The network forked at height " + std::to_string(upgrade) + ", please update your software: " + mevacoin::LATEST_VERSION_URL;
            }
        }
    ...
    std::string get_status_string(mevacoin::COMMAND_RPC_GET_INFO::response iresp)
    {
        std::stringstream ss;
