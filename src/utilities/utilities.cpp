...
namespace utilities
{
        ...
        const uint64_t currentHeight)
    {
        ...
        if (unlockTime >= mevacoin::parameters::MEVACOIN_MAX_BLOCK_NUMBER)
        {
            const uint64_t currentTimeAdjusted = static_cast<uint64_t>(std::time(nullptr)) + mevacoin::parameters::MEVACOIN_LOCKED_TX_ALLOWED_DELTA_SECONDS;

            return currentTimeAdjusted >= unlockTime;
        }

        const uint64_t currentHeightAdjusted = currentHeight + mevacoin::parameters::MEVACOIN_LOCKED_TX_ALLOWED_DELTA_BLOCKS;
    ...
    /* The formula for the block size is as follows. Calculate the
       ...
       src/MevaCoinCore/Currency.cpp::maxBlockCumulativeSize(). Call this value
       x.
    ...
    */
    ...
    {
        const uint64_t numerator = currentHeight * mevacoin::parameters::MAX_BLOCK_SIZE_GROWTH_SPEED_NUMERATOR;
        const uint64_t denominator = mevacoin::parameters::MAX_BLOCK_SIZE_GROWTH_SPEED_DENOMINATOR;

        const uint64_t growth = numerator / denominator;

        const uint64_t x = mevacoin::parameters::MAX_BLOCK_SIZE_INITIAL + growth;
    ...
    uint64_t scanHeightToTimestamp(const uint64_t scanHeight)
    {
        ...
        uint64_t timestamp = mevacoin::parameters::GENESIS_BLOCK_TIMESTAMP + secondsSinceLaunch;
    ...
    uint64_t timestampToScanHeight(const uint64_t timestamp)
    {
        ...
        if (timestamp <= mevacoin::parameters::GENESIS_BLOCK_TIMESTAMP)
        {
            return 0;
        }

        /* Find the amount of seconds between launch and the timestamp */
        uint64_t launchTimestampDelta = timestamp - mevacoin::parameters::GENESIS_BLOCK_TIMESTAMP;

        /* Get an estimation of the amount of blocks that have passed before the
           timestamp */
        return launchTimestampDelta / mevacoin::parameters::DIFFICULTY_TARGET;
    }
