...
namespace mevacoin
{
    ...
    void WalletGreen::sendTransaction(const mevacoin::Transaction &mevaCoinTransaction)
    {
        ...
        syst::RemoteContext<void> relayTransactionContext(m_dispatcher, [this, &mevaCoinTransaction, &ec, &completion]()
                                                          { m_node.relayTransaction(mevaCoinTransaction, [&ec, &completion, this](std::error_code error)
                                                                                    {
      ec = error;
      this->m_dispatcher.remoteSpawn(std::bind(asyncRequestCompletion, std::ref(completion))); }); });
        relayTransactionContext.get();
        ...
        if (ec)
        {
            m_logger(ERROR, BRIGHT_RED) << "Failed to relay transaction: " << ec << ", " << ec.message() << ". Transaction hash " << getObjectHash(mevaCoinTransaction);
            throw std::system_error(ec);
        }
    }
    ...
    size_t WalletGreen::validateSaveAndSendTransaction(const ITransactionReader &transaction, const std::vector<WalletTransfer> &destinations, bool isFusion, bool send)
    {
        ...
        if (mevaCoinTransaction.extra.size() >= mevacoin::parameters::MAX_EXTRA_SIZE_V2)
        {
            m_logger(ERROR, BRIGHT_RED) << "Transaction extra is too large. Allowed: "
                                        << mevacoin::parameters::MAX_EXTRA_SIZE_V2
                                        << ", actual: " << mevaCoinTransaction.extra.size()
                                        << ".";
