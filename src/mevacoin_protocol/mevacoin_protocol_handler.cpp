...
#include "mevacoin_protocol_handler.h"
...
namespace mevacoin
{
    ...
    MevaCoinProtocolHandler::MevaCoinProtocolHandler(const Currency &currency, syst::Dispatcher &dispatcher, ICore &rcore, IP2pEndpoint *p_net_layout, std::shared_ptr<logging::ILogger> log) : m_dispatcher(dispatcher),
                                                                                                                                                                                                    m_currency(currency),
                                                                                                                                                                                                    m_core(rcore),
                                                                                                                                                                                                    m_p2p(p_net_layout),
    ...
    CoreStatistics MevaCoinProtocolHandler::getStatistics()
    {
    ...
    void MevaCoinProtocolHandler::log_connections()
    {
        ...
        m_p2p->for_each_connection([&](const MevaCoinConnectionContext &cntxt, uint64_t peer_id)
    ...
    bool MevaCoinProtocolHandler::process_payload_sync_data(const CORE_SYNC_DATA &hshd, MevaCoinConnectionContext &context, bool is_initial)
    {
        ...
        else
        {
            ...
            ss << "Your " << MEVACOIN_NAME << " node is syncing with the network ";
...
#define HANDLE_NOTIFY(CMD, Handler)                                                                                                           \
    case CMD::ID:                                                                                                                             \
    {                                                                                                                                         \
        ret = notifyAdaptor<CMD>(in, ctx, [this](int a1, CMD::request &a2, MevaCoinConnectionContext &a3) { return Handler(a1, a2, a3); }); \
        break;                                                                                                                                \
    }
...
#undef HANDLE_NOTIFY
    ...
    int MevaCoinProtocolHandler::processObjects(MevaCoinConnectionContext &context, std::vector<RawBlock> &&rawBlocks, const std::vector<CachedBlock> &cachedBlocks)
    {
    ...
    int MevaCoinProtocolHandler::doPushLiteBlock(NOTIFY_NEW_LITE_BLOCK::request arg, MevaCoinConnectionContext &context, std::vector<BinaryArray> missingTxs)
    {
    ...
    bool MevaCoinProtocolHandler::request_missing_objects(MevaCoinConnectionContext &context, bool check_having_blocks)
    {
        ...
        else
        {
            ...
            logger(logging::INFO, logging::BRIGHT_YELLOW) << context << "Successfully synchronized with the "
                                                          << mevacoin::MEVACOIN_NAME << " Network.";
            on_connection_synchronized();
        }
        return true;
    }
    ...
    bool MevaCoinProtocolHandler::on_connection_synchronized()
    {
        ...
        if (m_synchronized.compare_exchange_strong(val_expected, true))
        {
            logger(logging::INFO)
                << ENDL;
            logger(INFO, BRIGHT_CYAN) << "===[ " + std::string(mevacoin::MEVACOIN_NAME) + " Tip! ]=============================" << ENDL;
            ...
            m_observerManager.notify(&IMevaCoinProtocolObserver::blockchainSynchronized, m_core.getTopBlockIndex());
        }
        return true;
    }
    ...
    int MevaCoinProtocolHandler::handleRequestTxPool(int command, NOTIFY_REQUEST_TX_POOL::request &arg,
    ...
    void MevaCoinProtocolHandler::relayBlock(NOTIFY_NEW_BLOCK::request &arg)
    {
        ...
        m_p2p->for_each_connection([this, &liteBlockConnections, &normalBlockConnections](const MevaCoinConnectionContext &ctx, uint64_t peerId)
                                   {
    ...
    void MevaCoinProtocolHandler::relayTransactions(const std::vector<BinaryArray> &transactions)
    {
    ...
    void MevaCoinProtocolHandler::recalculateMaxObservedHeight(const MevaCoinConnectionContext &context)
    {
        ...
        m_p2p->for_each_connection([&peerHeight, &context](const MevaCoinConnectionContext &ctx, uint64_t peerId)
                                   {
    ...
    uint32_t MevaCoinProtocolHandler::getBlockchainHeight() const
    {
        std::lock_guard<std::mutex> lock(m_blockchainHeightMutex);
