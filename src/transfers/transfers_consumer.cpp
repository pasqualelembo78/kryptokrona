...
namespace mevacoin
{
    ...
    TransfersConsumer::TransfersConsumer(const mevacoin::Currency &currency, INode &node, std::shared_ptr<logging::ILogger> logger, const SecretKey &viewSecret) : m_node(node), m_viewSecret(viewSecret), m_currency(currency), m_logger(logger, "TransfersConsumer")
    {
        updateSyncStart();
    }
